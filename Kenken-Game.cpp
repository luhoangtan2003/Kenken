...
int Check_Cage(Kenken Board, Coord Point){
    ...
    switch(Board.Cages[Index_Cage].Operation){
        case '+':{
            ...
            int DK_1 = 0;
            if(Total==Value && Is_Full_Cage(Board,Index_Cage)){
                DK_1 = 1;
            }
            int DK_2 = 0;
            if(Total<Value && !Is_Full_Cage(Board,Index_Cage)){
...
void Show_Cage(Kenken Board){
	...
	for(int i=0;i<GRID;i++){
		...
		for(int j=0;j<GRID;j++){
			...
			printf("%5d%c|",Board.Cages[Get_Cage(Board,Point)].Value_Of_Cage,Board.Cages[Get_Cage(Board,Point)].Operation);
		}
		printf("\n");
	}
	printf("\n");
...
int main(int argc, char const *argv[]){
    ...
    printf("Số trạng thái: %d",Num_Of_State);
}